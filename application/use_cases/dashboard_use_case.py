...
class GenerateFanDashboardUseCase:
    ...
    def execute(self, user_id: str) -> FanDashboardSnapshot:
        ...
        # Recuperar las instancias de los partidos en una sola consulta batch (evitando N+1 queries)
        ...
        for match in matches:
            ...
            # Conteos de competiciones
            competition_counts[match.competition.name] += 1

        # Cálculo de rendimientos según la instrucción
        puntos_obtenidos = (wins * 3) + (draws * 1)
        puntos_posibles = total_matches * 3
        
        # win_percentage como rendimiento (puntos) o porcentaje de triunfos, 
        # en este caso se pidió que se asigne el % de rendimiento al dashboard
        win_percentage = (puntos_obtenidos / puntos_posibles) * 100.0
        
        # undefeated_percentage es el porcentaje de partidos que no se perdieron
        undefeated_percentage = ((wins + draws) / total_matches) * 100.0

        # Estadios más visitados ordenados de mayor a menor
